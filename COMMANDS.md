...
| Command | Description |
|---|---|
| `:CW train <title\|slug\|url> [language]` | Open a kata by title (as the site shows it, e.g. "Unique In Order"), slug, or URL. A trailing word that names a language is the language; quote the title (`"Learn Python"`) when its last word is one |
